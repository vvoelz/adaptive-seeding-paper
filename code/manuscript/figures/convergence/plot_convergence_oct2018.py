...
## NOTE: inner dimension is tau_MLE, tau_MLE_known_pi, tau_weighted_MLE, tau_rownorm, tau_corrected, tau_OOM
...
labels = ['MLE', 'MLE known_pi', 'MLE pop-weighted', 'row-norm', 'corrected', 'OOM']
colors = ['r','b','m','y','c','g']
linewidth = 1.5
markersize = 4
...
for ntraj_index in range(len(trajlengths)):
    ...
    for i in range(len(labels)-1): # one less label because there is no OOM delF 
...
plt.tight_layout()    
plt.savefig('convergence_oct2018.pdf')
